...
- **Per-project breakdown** — which codebases are eating your budget. 30-day trend analysis.
...
Design multi-agent systems visually. Drag agents onto a canvas. Connect them. Define roles, capabilities, restrictions. Export as a CLAUDE.md that Claude Code follows.

- **Visual canvas** — React Flow powered. Supervisors, workers, delegation chains.
...
4. **Permission flow** — `PreToolUse` hooks POST to Forge. The UI renders Allow/Deny. The hook polls for your decision. 5-minute timeout. If Forge is unreachable, Claude keeps going (fail-open).
